...
(function () {
    var root = this;
    var standaloneTree = root.standaloneTree = {
	about: {
	    name: "tree",
	    title: "Tree",
	...
	create: function (params) {
	    var renderer = this;
	    if (! window.hasOwnProperty('rendererTree')) {
		window.rendererTree = [];
	    }
	    ...
	    window.rendererTree.push(instance);
...
.tree-node-selected {\
  ...
  border-radius: 3px;\
		...
		document.getElementById('tree_search_input_'+index).addEventListener('keypress', function (event) {
		    ...
		    if (event.keyCode == '13') {
			window.rendererTree[index].goTo(index);
		    }
		});
	...
	redraw: function (index) {
	    var renderer = window.rendererTree[index];
	...
	renderNode: function (params) {
		...
		if (params.node.childNodes.length) {
			...
			html += "<span style='cursor: pointer; margin-right: 5px;' onclick='window.rendererTree["+index+"].settings.data.nodes[\""+params.node.id+"\"].expanded=false;window.rendererTree["+index+"].redraw("+index+");'>&dtrif;</span>";
		    } else {
			html += "<span style='cursor: pointer; margin-right: 5px;' onclick='window.rendererTree["+index+"].settings.data.nodes[\""+params.node.id+"\"].expanded=true;window.rendererTree["+index+"].redraw("+index+");'>&rtrif;</span>";
		    }
		} else {
		...
		html += "<span "+tooltip+"onclick='window.rendererTree["+index+"].selectNode("+index+", \""+params.node.id+"\");'>"+label+"</span>";

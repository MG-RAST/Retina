function xlsx(file) {
	...
	if (zip.file('xl/sharedStrings.xml')) {
	    ...
	    promises.push(p1);
	    zip.file('xl/sharedStrings.xml').async("string").then(function success(content) { texts.sharedStrings = content.split(/<t.*?>/g); p1.resolve(); });
	}
	if (zip.file('docProps/core.xml')) {
	    var p2 = jQuery.Deferred();
	    promises.push(p2);
	    zip.file('docProps/core.xml').async("string").then(function success(content) { texts.core = content; p2.resolve(); });
	}
	if (zip.file('xl/workbook.xml')) {
	    var p3 = jQuery.Deferred();
	    promises.push(p3);
	    zip.file('xl/workbook.xml').async("string").then(function success(content) { texts.workbook = content; p3.resolve(); });
	}
	if (zip.file('xl/styles.xml')) {
	    var p4 = jQuery.Deferred();
	    promises.push(p4);
	    zip.file('xl/styles.xml').async("string").then(function success(content) { texts.styles = content.split('<numFmt '); p4.resolve(); });
	}
	...
	jQuery.when.apply(this, promises).then(function() {
	    ...
	    if (texts.hasOwnProperty('sharedStrings')) { // Process sharedStrings
		var s = texts.sharedStrings;
		var i = s.length;
	    ...
	    if (texts.hasOwnProperty('core')) { // Get file info from "docProps/core.xml"
		var s = texts.core;
	    ...
	    if (texts.hasOwnProperty('workbook')) { // Get workbook info from "xl/workbook.xml" - Worksheet names exist in other places, but "activeTab" attribute must be gathered from this file anyway
		var s = texts.workbook;
		var index = s.indexOf('activeTab="');
		if (index > 0) {
		...
		while (--i) { // Do not process i === 0, because s[0] is the text before the first sheet element
		    var id = s[i].substr(s[i].indexOf('name="') + 6);
		    result.worksheets.unshift({ name: id.substring(0, id.indexOf('"')), data: [] });
		}
	    }

	    if (texts.hasOwnProperty('styles')) { // Get style info from "xl/styles.xml"
		var s = texts.styles;
		var i = s.length;
		var t, f;
		...
		while (--i) {
		    var id = getAttr(s[i], 'numFmtId'); f = numFmts[id];
		...
		while (i--) {
		    ...
		    var t = getAttr(s[0].substr(s[0].indexOf('<dimension')), 'ref');
			...
			var row = w[+getAttr(s[j], 'r') - 1] = [];
			var columns = s[j].split('<c ');
